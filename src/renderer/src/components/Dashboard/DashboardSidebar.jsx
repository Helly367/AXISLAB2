...
const DashboardSidebar = ({
    ...
    ], [membersCount, materielsCount, tasksEnCours, campagnesEnCours, notificationsCount]);
    ...
    return (
        <div
            className={`h-full ${collapsed ? "w-20" : "w-60"
                } bg-white shadow-lg flex flex-col transition-all duration-300 ease-in-out`}
        >
            {/* HEADER */}
            <div className="flex items-center justify-between p-4 flex-shrink-0">
                {!collapsed && (
                    <h2 className="text-lg font-bold text-blue-600 transition-opacity duration-200">
                        Axis Lab
                    </h2>
                )}
