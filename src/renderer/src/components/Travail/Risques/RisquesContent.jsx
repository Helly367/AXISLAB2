...
const RisquesContent = ({ onUpdateRisks }) => {
    ...
    const calculateRiskLevel = (probabilite, impact) => {
        const score = Number(probabilite) * Number(impact);
    ...
    const getStatusInfo = (statut) => {
        switch (statut) {
            case 'actif': return { color: 'bg-red-100 text-red-800', icon: <Warning className="text-red-600" />, label: 'Actif' };
            case 'en_traitement': return { color: 'bg-yellow-100 text-yellow-800', icon: <Assignment className="text-yellow-600" />, label: 'En traitement' };
            case 'resolu': return { color: 'bg-green-100 text-green-800', icon: <CheckCircle className="text-green-600" />, label: 'Résolu' };
            case 'ignore': return { color: 'bg-gray-100 text-gray-800', icon: <Block className="text-gray-600" />, label: 'Ignoré' };
            default: return { color: 'bg-gray-100 text-gray-800', icon: <Info className="text-gray-600" />, label: statut };
        }
    };

    const getRiskLevelInfo = (niveau) => {
        switch (niveau) {
            case 'critique': return { color: 'bg-red-600 text-white', label: 'Critique' };
            case 'élevé': return { color: 'bg-orange-500 text-white', label: 'Élevé' };
            case 'moyen': return { color: 'bg-yellow-500 text-white', label: 'Moyen' };
            case 'faible': return { color: 'bg-green-500 text-white', label: 'Faible' };
            default: return { color: 'bg-gray-500 text-white', label: niveau };
        }
    };

    const getCategorieIcon = (categorie) => {
        switch (categorie) {
            case 'technique': return <span className="text-blue-600">🔧</span>;
            case 'financier': return <span className="text-green-600">💰</span>;
            case 'ressources': return <span className="text-purple-600">👥</span>;
            case 'externe': return <span className="text-orange-600">🌍</span>;
    ...
    const stats = {
        ...
        critique: risks.filter(r => r.probabilite * r.impact >= 0.5).length,
        parCategorie: {
    ...
    const handleAddRisk = (newRisk) => {
        const riskWithId = {
            ...newRisk,
            id: Date.now(),
            niveau: calculateRiskLevel(Number(newRisk.probabilite), Number(newRisk.impact)),
            actions: []
        };
    ...
    const handleEditRisk = (updatedRisk) => {
        const updatedRisks = risks.map(r =>
            r.id === updatedRisk.id
                ? { ...updatedRisk, niveau: calculateRiskLevel(Number(updatedRisk.probabilite), Number(updatedRisk.impact)) }
                : r
        );
        setRisks(updatedRisks);
        onUpdateRisks?.(updatedRisks);
    ...
    const filteredRisks = risks.filter(risk =>
        (filter === 'all' || risk.statut === filter) &&
        (filterCategorie === 'all' || risk.categorie === filterCategorie)
    );

    const sortedRisks = [...filteredRisks].sort((a, b) => {
        return (b.probabilite * b.impact) - (a.probabilite * a.impact);
    });
        ...
        <div className="w-full bg-gray-200 p-4">
            ...
            <div className="bg-white rounded-lg shadow-md p-2 mb-4 flex gap-2">
                <button
                    ...
                    <Warning fontSize="small" /> Liste des risques
                </button>
                <button
                    ...
                    <Timeline fontSize="small" /> Matrice d'impact
                </button>
            </div>
            ...
            {viewMode === 'list' ? (
                <div className="space-y-4">
                    {sortedRisks.map((risk) => {
                        ...
                        return (
                            <div key={risk.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow p-6 relative group m-4">
                                {/* Actions */}
                                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleViewRiskDetails(risk)} className="p-2 bg-blue-100 text-blue-600 rounded-full hover:bg-blue-200" title="Voir détails"><Info fontSize="small" /></button>
                                    <button onClick={() => handleOpenEditModal(risk)} className="p-2 bg-blue-100 text-blue-600 rounded-full hover:bg-blue-200"><Edit fontSize="small" /></button>
                                    <button onClick={() => handleDeleteRisk(risk.id)} className="p-2 bg-red-100 text-red-600 rounded-full hover:bg-red-200"><Delete fontSize="small" /></button>
                                </div>

                                {/* Header */}
                                <div className="flex items-start gap-4 mb-4">
                                    <div className={`px-3 py-1 rounded-full text-sm font-medium ${levelInfo.color}`}>{levelInfo.label}</div>
                                    <div className={`px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1 ${statusInfo.color}`}>{statusInfo.icon}{statusInfo.label}</div>
                                    <div className="flex items-center gap-1 text-gray-500 text-sm">{getCategorieIcon(risk.categorie)}<span className="capitalize">{risk.categorie}</span></div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-4">
                                    <div>
                                        ...
                                        <div className="w-full bg-gray-200 rounded-full h-2">
                                            <div className="bg-blue-500 rounded-full h-2" style={{ width: `${risk.probabilite * 100}%` }} />
                                        </div>
                                    </div>
                                    <div>
                                        <div className="flex justify-between items-center mb-1">
                                        ...
                                        <div className="w-full bg-gray-200 rounded-full h-2">
                                            <div className="bg-orange-500 rounded-full h-2" style={{ width: `${risk.impact * 100}%` }} />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mb-4">
                                    <h4 className="font-medium text-gray-700 mb-2 flex items-center gap-2">
                                        <Assignment className="text-blue-600" fontSize="small" /> Plan de mitigation
                                    </h4>
                                    <p className="text-gray-600 text-sm bg-blue-50 p-3 rounded-lg">{risk.plan_mitigation}</p>
                                </div>

                                <div className="flex items-center justify-between text-sm text-gray-500 border-t pt-4">
                                    <div className="flex items-center gap-2"><span className="font-medium">Responsable:</span> {risk.responsable}</div>
                                    ...
                                    <div className="mt-4 pt-4 border-t">
                                        ...
                                        <div className="flex gap-2 flex-wrap">
                                            {risk.actions.map((action, index) => {
                                                const desc = action.description.length > 30 ? action.description.substring(0, 30) + "..." : action.description;
                                                return (
                                                    <span key={index} className={`text-xs px-2 py-1 rounded-full ${action.completed ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
            ...
            {/* Modals */}
            <ModalAddRisk isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSave={handleAddRisk} />
            <ModalEditRisk isOpen={isEditModalOpen} onClose={() => { setIsEditModalOpen(false); setRiskToEdit(null); }} onSave={handleEditRisk} riskToEdit={riskToEdit} />
            <RiskDetails isOpen={isDetailsModalOpen} onClose={() => { setIsDetailsModalOpen(false); setSelectedRisk(null); }} risk={selectedRisk} onUpdate={handleEditRisk} />
        </div>
    );
};

export default RisquesContent;