...
const TachesDetails = ({ isOpen, onClose, task, onUpdateStatus, onToggleSubtask, members, allTasks }) => {
    ...
    const handleAddComment = () => {
        ...
        if (trimmedComment) {
            // Exemple de logique : on pourrait appeler une fonction pour ajouter le commentaire
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <div className="bg-gray-50 rounded-lg p-3">
                            ...
                            <p className="font-medium flex items-center gap-1">
                                ...
                                {task.date_echeance ? new Date(task.date_echeance).toLocaleDateString('fr-FR') : 'Non définie'}
                            </p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3">
                                ...
                                {task.dependances.map((depId) => {
                                    ...
                                    return (
                                        <div key={depId} className="p-2 bg-gray-50 rounded-lg text-sm">
                                            {depTask ? depTask.titre : `Tâche #${depId}`}
                                        </div>
                                    );
                                })}
                            </div>
                    ...
                    <div className="border-t pt-4 flex justify-between items-center">
                        <div className="flex gap-2">
                            {['a_faire', 'en_cours', 'en_attente', 'termine'].map((status) => (
                                <button
                                    key={status}
                                    onClick={() => onUpdateStatus(task.id, status)}
                                    className={`px-3 py-1 rounded-lg text-sm ${task.statut === status
                                        ? 'bg-' + (status === 'a_faire' ? 'gray' : status === 'en_cours' ? 'blue' : status === 'en_attente' ? 'yellow' : 'green') + '-600 text-white'
                                        : 'bg-' + (status === 'a_faire' ? 'gray' : status === 'en_cours' ? 'blue' : status === 'en_attente' ? 'yellow' : 'green') + '-200'
                                        }`}>
                                    {status === 'a_faire' ? 'À faire' : status === 'en_cours' ? 'En cours' : status === 'en_attente' ? 'En attente' : 'Terminé'}
                                </button>
                            ))}
                        </div>
                        <button
