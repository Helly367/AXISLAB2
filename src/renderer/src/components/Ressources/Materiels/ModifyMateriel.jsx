...
const ModifyMateriel = ({ isOpen, onClose, project, budget, materielVoire }) => {


    const { phases, loadPhases, setPhases } = usePhases();
    const { createMateriel, updateMateriel  } = useMateriels();
    ...
    const PhasesDuProjet = useMemo(() => {
        ...
        return phases.filter(m => Number(m.projet_id) === Number(project.projet_id));
    ...
    // Charger les Phases quand la modale s'ouvre
    useEffect(() => {
        if (isOpen && materielVoire) {
            reset({
                nom: materielVoire.nom || '',
                categorie: materielVoire.categorie || '',
                prix: materielVoire.prix || '',
                quantite: materielVoire.quantite || 1,
                phase_id: materielVoire.phase_id,
                statut: materielVoire.statut || 'en_attente',
                description: materielVoire.description || '',
                fournisseur: materielVoire.fournisseur || ''
            });
        }
    }, [isOpen, materielVoire, reset]);
    ...
    const filterBudgetPhase = useMemo(() => {
        if (!phases || !selectedPhaseId) return null;
        const phase = phases.find(m => Number(m.phase_id) === Number(selectedPhaseId));
        return phase;

    }, [phases, selectedPhaseId])

    const devise = budget?.devise;
    const budget_restant = Number(filterBudgetPhase?.budget_restant || 0);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const newMateriel = {
                ...data,
                materiel_id: materielVoire.materiel_id,
            ...
            const result = await updateMateriel(project?.projet_id, newMateriel);
        ...
        <div className="fixed inset-0 bg-opacity flex items-center justify-center z-50 p-4">
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
                        ...
                        {loadingPhases ? (
                            ...
                            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
                                ...
                                <p className="text-sm text-yellow-600 mb-4">
                                    Vous devez d'abord ajouter des Phases à ce projet.
                                </p>
                                <button
                                    type="button"
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                            <PlayArrow className="inline mr-2 text-blue-600" />
                            Statut
                        </label>

                        <select
                            {...register('statut')}
                            className={`${style} ${verifieChamps(errors, watchedFields, 'statut')} `}
                        >
                            <option value="en_attente">En attente</option>
                            <option value="dispinible">Dispinible</option>
                    ...
                    <div className="flex justify-end gap-3 border-t pt-4">
                        ...
                        <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            className='bg-primary text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed'
                        >
